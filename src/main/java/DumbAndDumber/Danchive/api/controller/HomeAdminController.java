...
public class HomeAdminController {
    ...
    public ResponseEntity<ApiResponse<List<BannerDto>>> listBanners() {
        return ResponseEntity.ok(ApiResponse.ok("배너 목록 조회 성공", adminService.listBanners()));
    }

    @PreAuthorize("hasRole('ADMIN')")
    @PostMapping("/banners")
    public ResponseEntity<ApiResponse<BannerDto>> createBanner(@RequestBody BannerUpsertRequest req) {
        return ResponseEntity.status(201).body(ApiResponse.ok("배너가 생성되었습니다.", adminService.createBanner(req)));
    }
    ...
    public ResponseEntity<ApiResponse<BannerDto>> updateBanner(@PathVariable Long id, @RequestBody BannerUpsertRequest req) {
        return ResponseEntity.ok(ApiResponse.ok("배너가 수정되었습니다.", adminService.updateBanner(id, req)));
    }
    ...
    public ResponseEntity<ApiResponse<List<NewsDto>>> listNews() {
        return ResponseEntity.ok(ApiResponse.ok("뉴스 목록 조회 성공", adminService.listNews()));
    }

    @PreAuthorize("hasRole('ADMIN')")
    @PostMapping("/news")
    public ResponseEntity<ApiResponse<NewsDto>> createNews(@RequestBody NewsUpsertRequest req) {
        return ResponseEntity.status(201).body(ApiResponse.ok("뉴스가 생성되었습니다.", adminService.createNews(req)));
    }
    ...
    public ResponseEntity<ApiResponse<NewsDto>> updateNews(@PathVariable Long id, @RequestBody NewsUpsertRequest req) {
        return ResponseEntity.ok(ApiResponse.ok("뉴스가 수정되었습니다.", adminService.updateNews(id, req)));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> deleteNews(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(ApiResponse.ok("뉴스가 삭제되었습니다."));
    }
}
